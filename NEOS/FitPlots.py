...
def getChi2(mass,angl,wave_packet = False):
    ...
    chi2 = fitter.get_both_chi2(model)
    return chi2
...
figBF.suptitle(r'Best fit:  $\Delta m^2_{41} = %.2f eV^2$, $\sin^2 2\theta_{14} = %.3f$. Total $\chi^2 = %.2f$'%(bestfit[0],bestfit[1], bestfit[index_chi]))
...
# We find which is the chi2 of the null hypothesis
null_hyp_WP = getChi2(0,0, wave_packet = True)[index_chi-2]
...
cont_PW = ax_comp.tricontour(data_PW[:,1],data_PW[:,0],(data_PW[:,3]-null_hyp_PW),levels = [2.30,6.18], colors = 'red', linestyles = ['dotted','solid'])
