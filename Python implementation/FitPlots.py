...
axSt.contour(data[:,0].reshape(nmas,nang),data[:,1].reshape(nmas,nang),(data[:,2]-np.min(data[:,2])).reshape(nmas,nang),levels = [2.30,6.18,11.83])
# axSM.scatter(,0.07821,marker = '+', label = 'Coherent best fit')
axSt.scatter(bestfit[0],bestfit[1],marker = '+', label = 'Our best fit')
...
axSt.set_xscale('log')
axSt.set_yscale('log')
axSt.set_xlabel(r"$\Delta m^2_{13} (eV^2)$", fontsize = 16)
axSt.set_ylabel(r"$\sin^2 2 \theta_{13}$", fontsize = 16)
